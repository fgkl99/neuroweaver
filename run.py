...
def compute_fs_from_t(t: pd.Series) -> float | None:
    # robust-ish fs estimate from median dt
    dt = t.diff().dropna()
    if len(dt) < 5:
        return None
    med = float(dt.median())
...
def main():
    root = Path(".")
    meta_path = root / "data" / "recording_001.meta.json"
    ...
    if columns_ok:
        ...
        # allow up to 0.5% out-of-range before flagging
...
def grade_quality(c: Checks) -> str:
    # Brutal grading: if any critical check fails → poor
    critical = [
        c.schema_version_ok,
        c.channels_ok,
        c.columns_ok,
